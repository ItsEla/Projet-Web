<?PHP
include_once 'C:\Users\hp\AppData\Local\Microsoft\Windows\Burn\Burn\xampp\htdocs\Melodyo\Core\config.php';
class categorieC {
	/*
...
*/	
	...
	function getNoP($id){
        $sql="SELECT count(*) FROM `categorie` WHERE `categorie`.`id_cat` = ".$id;
        $db = config::getConnexion();
        try{
        $liste=$db->query($sql);
        return $liste;
    ...
    public static function getTopcategorie(){
        $sql="SELECT categorie.id, count(categorie.id) FROM categorie  GROUP by categorie.id ORDER by count(categorie.id) desc";
        $db = config::getConnexion();
        try{
        $liste=$db->query($sql);
        return $liste;
